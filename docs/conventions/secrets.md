...
다음 파일 패턴은 **어떤 이유로도** 읽지 않는다. 읽으면 해당 내용이 AI 컨텍스트, 로그, 후속 응답, 또는 커밋 diff에 노출될 수 있다. 이 정책은 사용자 승인으로도 해제되지 않는다 (자세한 이유는 [`ai-guardrails.md`](./ai-guardrails.md) 섹션 1의 "왜 승인 카테고리를 두지 않는가" 참조).

| 패턴 | 이유 |
|------|------|
| `.env`, `.env.local`, `.env.production`, `.env.*` | 실제 시크릿 값을 평문으로 포함한다. |
| `*.pem`, `*.key`, `id_rsa*` | 개인키 / TLS 인증서 개인키. 유출 시 재발급 외엔 복구 불가. |
| `credentials.json`, `*credentials*.json`, `service-account*.json` | 클라우드 서비스 자격증명 (GCP service account 등). JSON 형식 한정 — `credentials-flow.md` 같은 일반 문서는 별개. |
...
**예외:** `.env.example`은 read/write 모두 허용된다. 이 파일은 정의상 **실제 값이 없는 템플릿**이기 때문이다. 이 예외는 *파일 패턴 자체*가 안전한 경우이며, "사용자 승인에 의한 예외"와는 다른 카테고리다.
...
4. 사용자가 "그래도 읽어 달라"고 요청해도 **거절**한다. 사용자가 정말 그 내용을 봐야 한다면 본인이 직접 cat이나 편집기로 보면 된다 — AI 에이전트의 read 도구를 거칠 이유가 없다.

### 왜 승인 예외를 두지 않는가

"이번 한 번만 .env를 읽어서 설정 검증하자"는 유혹이 있을 수 있다. 하지만 한 번이라도 예외를 허용하면 다음 사고가 열린다: AI 컨텍스트는 turn 경계를 넘어 흘러가므로 한 번 본 시크릿이 후속 답변에 예시로 등장할 수 있고, 사용자가 그 답변을 그대로 commit·PR·로그에 복사할 수 있다. 또한 정책이 "조건부"가 되는 순간 reviewer/agent마다 다르게 해석되어 정책 자체가 무력화된다. 대안이 거의 항상 존재한다: `.env.example`의 키 구조를 보면 충분하고, 진짜 값이 필요한 경우는 사용자가 직접 확인하면 된다.
...
1. **절대** 시크릿을 `VITE_*` prefix로 노출하지 않는다 (브라우저 빌드 산출물에 평문으로 박힌다).
2. 백엔드 호출용 인증 토큰은 **백엔드에서 생성**한다. 클라이언트 빌드에 시크릿을 포함시키지 않는다.
3. `VITE_*` 변수는 원칙적으로 **URL·플래그·공개 ID**만 담는다.
4. 새 `VITE_*` 변수를 추가할 때는 "이 값이 브라우저 View Source로 보여도 괜찮은가?"를 스스로 물어라. "괜찮지 않다"면 이름부터 틀렸다.

### 토큰 전달 방식 — 이 프로젝트의 현재 상태

이 프로젝트는 MVP 보일러플레이트로서 **JWT를 `localStorage`에 저장하고 `Authorization: Bearer <token>` 헤더로 전달**한다 (`frontend/src/api/client.ts`, `frontend/src/context/AuthContext.tsx`). 이 패턴은:
- **장점**: 구현이 간단하고 입문자가 흐름을 따라가기 쉽다. CSRF 방어가 자동으로 따라온다(쿠키 자동 전송이 없으므로).
- **트레이드오프**: XSS 공격이 발생하면 JS가 `localStorage`에 접근 가능하므로 토큰이 탈취될 수 있다. 따라서 XSS 방어(React의 기본 escape, 외부 HTML 직접 렌더 금지)가 필수다.

**향후 보안 강화 옵션** (현재 MVP에는 적용하지 않음, 운영 단계로 넘어갈 때 검토):
- HttpOnly + Secure + SameSite=Strict 쿠키로 토큰 전달 → XSS로부터 토큰을 보호. 단 CSRF 토큰을 별도로 도입해야 한다.
- Refresh token 회전 패턴 → 짧은 만료 access token + 긴 만료 refresh token (HttpOnly 쿠키).
- Web Cryptography API 기반 토큰 암호화.

위 강화 옵션 도입 시점에는 이 문서의 "현재 상태" 섹션을 함께 갱신한다 (Phase 4-4 트리거: 인증 흐름 변경).
...
# 클라우드/서비스 자격증명 (JSON 형식 한정 — `credentials-flow.md` 같은
# 일반 문서 파일이 의도치 않게 차단되는 것을 막기 위해 `*.json` 접미사 명시)
credentials.json
*credentials*.json

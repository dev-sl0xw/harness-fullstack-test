...
> **권장 model:** `opus` (CLAUDE.md "에이전트 모델 정책" 참조). Codex 응답 필터링·심각도 분류·프로젝트 맥락 적용은 추론 깊이가 직접 가치로 변환되는 작업. must-fix를 nit으로 강등하거나 그 반대가 발생하면 PR 품질에 즉시 영향.
...
- **변경된 파일의 한국어 학습용 주석 품질 (이 프로젝트 고유 규칙)** — 파일 상단 역할/위치/관계, 함수별 목적/파라미터/흐름, 설계 의도("왜 이렇게 하는지")가 모두 갖춰져 있는지 점검. 결락 시 즉시 지적. CLAUDE.md "에이전트 모델 정책"이 시범 운영 중인 동안에는 sonnet 구현 에이전트의 산출물에서 주석 회귀가 발생할 가능성이 있으므로 평소보다 엄격하게 본다 — 결락 1건이라도 즉시 rollback 트리거로 분류

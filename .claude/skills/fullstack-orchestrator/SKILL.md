...
| solution-architect | `.claude/agents/solution-architect.md` | 아키텍처 설계·다이어그램·ADR·decisions.json | solution-architecture, architecture-diagrams | `docs/architecture/`, `_workspace/architecture/decisions.json` |
| cloud-infra-dev | `.claude/agents/cloud-infra-dev.md` | 클라우드 IaC 구현 (CDK/Terraform) | cloud-infra-build | `infra/{cloud}-{iac}/` |
...
- `solution-architect`는 **아키텍처 의사결정**(클라우드·IaC·컴퓨트·리전 선택) 담당. Phase 0-0에서 사용자 요청에 클라우드 키워드가 포함될 때만 실행된다. 결정을 `decisions.json`에 기록하고, 다이어그램을 `docs/architecture/`에 생성한다.
- `cloud-infra-dev`는 **IaC 코드 구현** 담당. Phase 2에서 `decisions.cloud_infra_dev.activate === true`일 때만 팀에 포함된다. `decisions.json`을 읽어 `infra/` 하위에 CDK/Terraform 코드를 생성한다.
...
4. 실행 모드 결정:
   ...
   - **코드 존재 + conventions 미존재** → **Phase 0-5(규칙 수립)를 자동 선행**한 뒤 사용자가 요청한 범위(부분 수정/전체 재구축)로 진행. 기존 코드는 그대로 두고 conventions만 신규 수립. 이후 구현 에이전트가 새로 수립된 conventions를 reference로 로드한 뒤 작업.
   - **코드 존재 + conventions 존재 + 사용자가 부분 수정 요청** → 부분 재실행. 해당 에이전트만 재호출
   - **코드 존재 + conventions 존재 + 새 입력/전체 재구축 요청** → 새 실행. 기존 `_workspace/`를 `_workspace_prev/`로 이동
   - **사용자가 규칙·컨벤션·가드레일 수립/수정 요청** (코드 유무 무관) → Phase 0-5만 실행

### Phase 0-0: Solution Architecture (조건부)

**언제 실행되는가:** 사용자 요청에 클라우드/배포/아키텍처 키워드가 포함될 때. 키워드 목록: `aws`, `gcp`, `azure`, `cloud`, `배포`, `deploy`, `인프라 설계`, `아키텍처`, `architecture`, `cdk`, `terraform`, `iac`, `serverless`, `ecs`, `fargate`, `lambda`, `cloud run`, `kubernetes`, `k8s`, `ec2`, `rds`, `cloud sql`, `s3`, `cloudfront`, `vpc`

**skip 조건:** 클라우드 키워드 없음 / 일반 기능 수정·버그 수정 / `docs/architecture/` 이미 존재 + 일반 작업

**트리거 매트릭스:**

| 코드 | conventions | architecture | 사용자 요청 | 실행 |
|------|-------------|-------------|-----------|------|
| ✗ | ✗ | ✗ | 신규 + 클라우드 키워드 | Phase 0-0 → 0-5 → 1→5 |
| ✗ | ✗ | ✗ | 신규 + 키워드 없음 | Phase 0-5 → 1→5 (skip) |
| ✓ | ✓ | ✗ | "클라우드 추가", "AWS 배포" | Phase 0-0 + cloud-infra-dev |
| ✓ | ✓ | ✓ | "아키텍처 수정" | Phase 0-0 audit 모드 |
| ✓ | ✓ | ✓ | 일반 수정 | skip |

**워크플로우:**

1. **solution-architect 스폰:**
   - `general-purpose` 타입, **`model: "opus"`** (검증·판단 에이전트)
   - 에이전트 정의 파일(`.claude/agents/solution-architect.md`)과 `solution-architecture` + `architecture-diagrams` 스킬을 읽도록 지시
   - 리더가 `initial_constraints` (사용자 대화에서 이미 결정된 항목) 를 JSON으로 전달

2. **Discovery Q&A:**
   - solution-architect가 코드 스캔으로 자동 도출 가능한 항목을 먼저 채움
   - 미확정 항목만 사용자에게 batch 질문 (3~5개씩)
   - MVP 기본값 세트 제공 (사용자가 "기본값" 응답 시 auto-진행)

3. **산출물 생성:**
   - `docs/architecture/` — 7개 다이어그램(.mmd + .md wrapper) + README.md
   - `docs/architecture/adr/` — Architecture Decision Records
   - `_workspace/architecture/decisions.json` — 구조화 결정 기록 (모든 후속 에이전트의 단일 진실원)

4. **후속 에이전트 연계:**
   - `decisions.json`의 `cloud_infra_dev.activate` 플래그에 따라 Phase 2에서 cloud-infra-dev 스폰 여부 결정
   - 리더에게 "Phase 0-5 실행 요청 + decisions.json 경로" SendMessage → project-architect가 클라우드 특화 컨벤션 갱신

5. **종료 조건:**
   - `docs/architecture/README.md` 생성됨
   - 7개 `.mmd` + 7개 `.md` wrapper 존재
   - ADR 파일 존재
   - `_workspace/architecture/decisions.json` 생성됨
...
2. **팀원 스폰**. 기본 4명 + `_workspace/architecture/decisions.json`의 `cloud_infra_dev.activate === true`이면 cloud-infra-dev 1명 추가 (총 5명). 각 에이전트는 작업 시작 전 **`docs/conventions/` 전체를 reference로 읽도록** 지시한다.
   ...
   **팀원 스폰 지시:**
   ...
   - `cloud-infra-dev` (조건부: `decisions.cloud_infra_dev.activate === true`일 때만): general-purpose 타입, **`model: "sonnet"`** (구현 에이전트). 에이전트 정의 파일의 역할과 스킬을 읽고, `_workspace/architecture/decisions.json` + `docs/architecture/deployment-*.mmd` + `docs/conventions/`를 reference로 로드한 뒤, IaC 코드를 생성하도록 지시. **backend-dev 완료 후에만 작업 시작** (Dockerfile + 환경변수 계약 필요).
...
트리거는 **추가/삭제**뿐 아니라 **의미 변경**(기존 항목의 역할·동작·범위가 바뀌는 경우)도 포함한다. "파일이 그대로 있으니 README 갱신 불필요"라는 판단은 위험하다 — 같은 파일의 *서술이 README에 반영되어 있다면* 해당 파일의 의미 변경도 트리거다.

| 변경 영역 | 갱신 필요 여부 | 갱신할 README 섹션 |
|---------|--------------|------------------|
| 새 에이전트 추가/삭제/**역할 변경** (`.claude/agents/*.md`) | **필수** | 하네스 구조 / 에이전트 목록 |
| 새 스킬 추가/삭제/**용도 변경** (`.claude/skills/*/SKILL.md`) | **필수** | 하네스 구조 / 스킬 목록 |
| **오케스트레이터 워크플로우 단계 추가/삭제/의미 변경** (Phase 신설, Phase 트리거 조건 변경) | **필수** | 워크플로우 단계 |
| **`.claude/skills/project-conventions/SKILL.md` 의미 변경** (8개 항목 내용/추가/삭제) | **필수** | 워크플로우 단계 / 가드레일 / 컨벤션 |
| **CLAUDE.md 가드레일 섹션 의미 변경** (read/write/exec/log 금지 패턴 추가/삭제/완화) | **필수** | 시스템 레벨 가드레일 |
| **`docs/conventions/*.md` 추가/삭제/의미 변경** (project-architect 산출물) | **필수** | 워크플로우 단계 / 컨벤션 |
...
| `docs/architecture/*` 추가/삭제/변경 (solution-architect 산출물) | **필수** | 프로젝트 구조 / 아키텍처 섹션 |
| `infra/{cloud}-{iac}/*` 추가/삭제/변경 (cloud-infra-dev 산출물) | **필수** | 프로젝트 구조 / 배포 섹션 |
| `_workspace/architecture/decisions.json` 의미 변경 | **선택** — 결정이 README의 "기술 스택/배포" 서술에 반영된 경우만 | 기술 스택 / 배포 |
| 일반 비즈니스 로직 변경 (핸들러 내부, 컴포넌트 내부 등) | 불필요 | — |
| 버그 수정, refactoring, 주석 보강 | 불필요 | — |
| lock 파일만 변경 | 불필요 | — |

**판단 기준:** "이 변경의 *서술*이 현재 README 어딘가에 있는가?"가 핵심 질문. 있다면 의미 변경이 필수 트리거다.
...
3. **리뷰 요청:**
   ...
   - **IaC 코드도 Codex 리뷰 범위에 포함**: IAM trust policy(OIDC 조건), Security Group(0.0.0.0/0 검사), SSM Parameter Store SecureString 사용 여부, User Data 스크립트의 secrets 노출 여부를 중점 점검.
...
### Phase 5: 정리 + 시범 운영 정책 평가

1. 모든 팀원에게 종료 요청 (SendMessage)
2. TeamDelete로 팀 정리
3. `_workspace/` 보존
4. **에이전트 모델 정책 — 시범 운영 평가 (조건부 실행):**
   - 조건: CLAUDE.md "에이전트 모델 정책" 섹션이 "시범 운영 중" 상태이고, 이번 빌드가 backend/frontend/infra 중 1개 이상에 sonnet으로 변경을 가한 **첫 sonnet 빌드**(또는 trial 연장 회차)인 경우
   - 측정: CLAUDE.md "시범 운영 평가 — 정량 기준" 표의 4개 기준을 모두 측정
     - qa miss를 reviewer가 후속 검출했는가? (1건이라도 → rollback)
     - 한국어 학습용 상세 주석 규칙이 결락된 파일이 있는가? (1개라도 → rollback)
     - must-fix 건수 직전(opus 빌드) 대비 +2건 이상 증가? (해당 시 → rollback)
     - qa 경계면 mismatch 건수 직전 대비 +50% 이상 증가? (해당 시 → rollback)
   - 결과 기록: `_workspace/model_policy_eval_{YYYY-MM-DD}.md`에 측정값과 판정(GO/rollback/연장) 기록
   - 후속 처리:
     - **GO (정책 확정)**: CLAUDE.md "시범 운영 중" → "확정"으로 갱신, 변경 이력에 평가 결과 기록, README 3개 언어판도 동기화 (Phase 4-4 자동 트리거)
     - **rollback**: 본 SKILL.md Phase 2의 backend/frontend/infra spawn 지시를 모두 `model: "opus"`로 되돌리고, 각 에이전트 정의 파일 권장 model 메타도 opus로 변경, CLAUDE.md / README 3개도 일괄 opus 표현으로 동기화. 변경 이력에 rollback 사유와 측정값 기록
     - **연장 (최대 2회)**: 측정값이 경계 영역이면 1회 추가 빌드 후 재평가. 변경 이력에 "연장 N회차" 기록. 총 3회 빌드(첫 trial + 연장 2회) 후에는 GO/rollback 둘 중 하나로 강제 결정
5. 사용자에게 결과 요약 보고:
   - 생성된 파일 목록
   - 빌드 결과
   - QA 검증 결과
   - 시범 운영 평가 결과 (조건부 실행 시)
   - 남은 작업 (있을 경우)
...
| Codex CLI 미설치/미인증/쿼터 소진 | code-reviewer가 `_workspace/review_report_*.md`에 **stub 보고서를 반드시 생성**(Phase 4-5 본문 참조). stub 필수 필드: `status: "Codex unavailable"`, `reason`, `retry_count`, `manual_review_performed`, `merge_recommendation`. stub 생성 후 Phase 5(PR body에 "Codex 리뷰 누락" 명시)로 진행 |
| Codex 응답 타임아웃 | 1회 재시도 → 재실패 시 code-reviewer가 수동 리뷰 수행, stub 보고서에 표기 후 Phase 5 진행 |
...
### 정상 흐름 B — 클라우드 추가
1. 사용자가 "현재 프로젝트에 AWS 배포 추가해줘" 요청
2. Phase 0에서 기존 코드 + conventions 존재 + architecture 미존재 → Phase 0-0 실행
3. Phase 0-0: solution-architect 스폰 → discovery Q&A → decisions.json + 7 다이어그램 + ADR
4. Phase 0-5: project-architect가 decisions.json 읽어 conventions 갱신
5. Phase 2: 기본 4명 + cloud-infra-dev (조건부)
6. Phase 3: cloud-infra-dev가 infra/aws-cdk/ 생성 (backend-dev 완료 후)
7. Phase 4: qa-engineer가 cdk synth 성공 확인
8. Phase 4-4: README 자동 갱신 (docs/architecture/, infra/ 트리거)
9. Phase 4-5: Codex 리뷰 (IaC 보안 중점)
10. Phase 5: PR 생성
11. 예상 결과: `docs/architecture/` + `infra/aws-cdk/` + `.github/workflows/deploy-aws.yml` 생성 + decisions.json 기반 conventions 갱신 + 모든 PR에 리뷰 보고서 첨부

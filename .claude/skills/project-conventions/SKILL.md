...
9. **RBAC 설계 원칙** — 역할 기반 접근 제어의 최소 권한·프론트↔백 일관성·확장 경로
...
- "나중에 복합 권한(ABAC, 리소스별 ACL)이 필요할 수 있으니" 다중 역할 조인 테이블을 미리 만든다 (지금은 user/admin 2가지 역할이면 충분).
...
| `credentials.json`, `*credentials*.json`, `service-account*.json` | 클라우드 자격증명 (JSON 형식 한정 — `credentials-flow.md` 같은 일반 문서 파일을 차단하지 않도록 `.json` 접미사 명시) |
...
**원리:** 각 사용자/역할에게는 업무 수행에 필요한 최소한의 권한만 부여한다.

**왜 중요한가:** 과도한 권한은 실수나 계정 탈취 시 피해 범위를 키운다. 기본 역할은 가장 적은 권한으로 시작하고, 필요할 때만 상위 역할을 부여한다.

**Go에서:**
- 새 API 엔드포인트의 기본 접근 수준은 "인증 필요"이다. "누구나 접근 가능"이나 "admin 전용"은 의식적으로 선택한다.
- 미들웨어 체인은 **deny by default**: 명시적으로 허용하지 않은 것은 거부.
- `RequireRole("admin")`은 admin만 허용. `RequireOwnerOrRole("admin")`은 본인 또는 admin 허용.

**React에서:**
- 프론트엔드의 역할 체크는 **UX 가이드일 뿐, 보안 경계가 아니다**. 백엔드 미들웨어가 진짜 보안 경계.
- 프론트에서 UI를 숨겨도, 사용자가 curl이나 devtools로 API를 직접 호출할 수 있다.
- 따라서 백엔드 미들웨어가 **반드시** 동일 수준의 검증을 수행해야 한다.

### 프론트↔백 일관성 원칙

역할 기반 접근 제어는 프론트와 백엔드 양쪽에서 동일한 수준으로 구현되어야 한다:

| 백엔드 보호 수준 | 프론트 UI | 불일치 시 위험 |
|----------------|---------|-------------|
| RequireRole("admin") | isAdmin 조건으로 UI 숨김 | 프론트만 숨기고 백엔드 미개입 → **보안 취약** |
| RequireOwnerOrRole("admin") | isOwner \|\| isAdmin 조건 | 프론트에서 admin UI를 안 보여주면 UX 문제 (기능은 있지만 접근 불가) |
| 미들웨어 없음 | 인증만 체크 | 정상 (공개 API) |

**qa-engineer 검증 포인트:** 위 매트릭스를 API 엔드포인트별로 교차 비교하여 불일치를 탐지한다.

### 단순 역할 모델 우선

**원리:** 역할 수가 3개 이하일 때는 users 테이블에 `role VARCHAR(20)` 컬럼 하나로 충분하다. 별도 roles 테이블이나 permissions 테이블은 다음 조건 중 하나가 발생할 때 도입한다:
- 역할이 4개 이상 필요 (user, editor, moderator, admin 등)
- 한 유저가 여러 역할을 동시에 가져야 함
- 역할별 세부 권한(permission)을 독립적으로 관리해야 함

**확장 경로:**
```
Phase 1: role VARCHAR 컬럼 (user/admin) — 현재 단계
Phase 2: roles 테이블 + user_roles 조인 — 역할 4개 이상 시
Phase 3: permissions 테이블 + role_permissions 조인 — 세부 권한 관리 시
```

**체크리스트:**
- [ ] 현재 필요한 역할이 3개 이하인가? → VARCHAR 컬럼으로 충분
- [ ] 한 유저가 여러 역할을 가져야 하는가? → 조인 테이블 필요
- [ ] 역할마다 다른 세부 권한이 필요한가? → permissions 테이블 필요
- [ ] 프론트에서 숨긴 UI가 백엔드에서도 동일하게 보호되는가?
...
| `docs/conventions/rbac.md` | 9 (RBAC 설계 원칙, 최소 권한, 확장 경로) |

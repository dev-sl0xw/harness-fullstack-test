...
| 인프라 (클라우드, 선택) | AWS CDK (TypeScript) / EC2 + Docker Compose / SSM Parameter Store |
...
> AWS 계정이 필요하다. 프리 티어 예상 비용: 월 ~$0.50 (Route 53 호스팅 영역만 과금).
> 프리 티어 종료 후 (12개월): 월 ~$25.

하네스는 `solution-architect`와 `cloud-infra-dev` 에이전트를 통해 선택적 AWS 배포를 지원한다:

1. 클라우드 배포 요청: *"이 프로젝트에 AWS 배포 추가해줘"*
2. Phase 0-0이 자동 트리거 — 아키텍처 설계·다이어그램·ADR 생성
3. CDK 프로젝트가 `infra/aws-cdk/`에 생성됨
4. 수동 배포: `cd infra/aws-cdk && npm install && npx cdk deploy --all --context env=dev`

아키텍처: 단일 EC2 (t3.micro) + Docker Compose + RDS + S3/CloudFront.
OIDC 역할 생성 등 상세 설정은 `infra/aws-cdk/README.md` 참조.
...
| `solution-architect` | 아키텍처 설계·C4 다이어그램·ADR·decisions.json (Phase 0-0, 클라우드 키워드 트리거 시) | 클라우드 키워드 트리거 시 |
| `cloud-infra-dev` | 클라우드 IaC 구현 — CDK/Terraform 코드 생성·검증 (Phase 2, 조건부) | 조건부 (클라우드만) |
...
| `solution-architecture` | 요구사항 정리·아키텍처 패턴·클라우드/IaC 비교·ADR 템플릿 | `solution-architect` |
| `architecture-diagrams` | Mermaid C4/시퀀스/배포/ER 다이어그램 패턴·네이밍 컨벤션 | `solution-architect` |
| `cloud-infra-build` | IaC 공통 원칙 + 클라우드+IaC 구현 가이드 (references/) | `cloud-infra-dev` |
